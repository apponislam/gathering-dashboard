        ...
        <div className="bg-slate-50 min-h-screen py-16 px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mx-auto">
                ...
                <div className="text-center mb-12">
                    ...
                    <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight sm:text-5xl">App Support Center</h1>
                    <p className="mt-4 text-lg text-slate-600">Need assistance with the Gathering app? Check our frequently asked questions below or contact us via email.</p>
                </div>
                    ...
                    <div className="space-y-4">
                        {faqs.map((faq, idx) => (
                            <div key={idx} className="bg-white border border-slate-200/80 rounded-2xl overflow-hidden transition-all duration-200">
                                <button type="button" onClick={() => toggleFaq(idx)} className="w-full px-6 py-5 text-left flex justify-between items-center hover:bg-slate-50/50 transition-colors">
                                    <span className="font-semibold text-slate-900 text-sm sm:text-base pr-4">{faq.question}</span>
                                    {openFaq === idx ? <ChevronUp className="h-5 w-5 text-[#412667] shrink-0" /> : <ChevronDown className="h-5 w-5 text-slate-400 shrink-0" />}
                                </button>

                                {openFaq === idx && <div className="px-6 pb-5 pt-1 text-slate-600 text-sm sm:text-base border-t border-slate-100/60 bg-slate-50/30 animate-in fade-in duration-200">{faq.answer}</div>}
                            </div>
                        ))}
                    </div>

                    {/* Extra Support Box */}
                    <div className="bg-linear-to-r from-[#412667] to-[#593985] text-white p-6 rounded-3xl shadow-sm mt-8 text-center">
                        ...
                        <div className="mt-4 flex flex-col sm:flex-row items-center justify-center gap-4 text-base font-semibold text-[#67E9F1]">
                            <div className="flex items-center gap-2">
                                ...
                                <a href="mailto:gatheringllc1@gmail.com" className="hover:underline">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <a href="tel:+19725617918" className="hover:underline">
                                    +1 (972) 561-7918
                                </a>
                            </div>
                        </div>

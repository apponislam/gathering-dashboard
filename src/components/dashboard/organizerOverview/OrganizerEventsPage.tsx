...
import { useGetUpcomingEventsQuery } from "@/redux/features/organizerdash/organizerDashApi";
import Link from "next/link";

export default function OrganizerEventsPage() {
    const { data: apiData, isLoading } = useGetUpcomingEventsQuery(undefined);
    ...
    return (
        <Card className="p-6">
                    ...
                    <tbody>
                        {upcomingEvents.length > 0 ? (
                            upcomingEvents.map((event: any) => {
                                const status = getStatus(event.startDate);
                                return (
                                    <tr key={event.id || event._id} className="border-b hover:bg-gray-50">
                                        <td className="px-6 py-4 text-sm text-gray-900">{event.title}</td>
                                        <td className="px-6 py-4 text-sm text-blue-600 capitalize">{event.category || "Uncategorized"}</td>
                                        <td className="px-6 py-4 text-sm text-gray-700">
                                            {formatDate(event.startDate)}
                                            {event.startTime && ` at ${event.startTime}`}
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 text-xs rounded ${status === "Upcoming" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}>{status}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <Link href={`/my-events/analytics/${event.id || event._id}`}>
                                                <button className="text-sm text-blue-600 hover:text-blue-800">View</button>
                                            </Link>
                                        </td>
                                    </tr>
            ...
            {/* Showing count without pagination */}

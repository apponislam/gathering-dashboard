...
//                     <p className="text-sm text-muted-foreground mt-1">Fill in the details to create your event</p>
//                 </DialogHeader>
...
//         const { id, value } = e.target;
//         setCoordinates((prev) => ({ ...prev, [id]: value }));
//         setError("");
//     };
...
//             tags: tagsArray.length > 0 ? tagsArray : ["tech", "innovation", "conference", "startups"], // Default tags
...
//             setError(error.data?.message || error.message || "Failed to create event. Please try again.");
//         }
//     };
...
//                         </Label>
//                         <Input id="address" placeholder="Enter venue or address" value={formData.address} onChange={handleInputChange} disabled={isLoading} />
//                     </div>
...
//                             </Label>
//                             <Input id="lat" type="number" step="0.000001" placeholder="23.8103" value={coordinates.lat} onChange={handleCoordinateChange} disabled={isLoading} />
//                         </div>
...
//                             </Label>
//                             <Input id="lng" type="number" step="0.000001" placeholder="90.4125" value={coordinates.lng} onChange={handleCoordinateChange} disabled={isLoading} />
//                         </div>
//                     </div>
...
//                                         {/* eslint-disable-next-line @next/next/no-img-element */}
//                                         <img src={imagePreview} alt="Preview" className="w-full h-32 object-cover" />
//                                         <Button type="button" variant="destructive" size="icon" className="absolute top-2 right-2 h-6 w-6 rounded-full" onClick={removeImage} disabled={isLoading}>
//                                             <X className="h-3 w-3" />
//                                         </Button>
//                                     </div>
//                                 </CardContent>
...
export function CreateEventModal({ open, onOpenChange }: CreateEventModalProps) {
    ...
    const handleSubmit = async () => {
        ...
        // Create event data object
        // Backend will generate coordinates from address automatically
        const eventData = {
            title: formData.title,
            description: formData.description,
            category: formData.category,
            ...
            locationType: formData.locationType as "physical" | "virtual" | "hybrid",
            address: formData.address,
            placeId: selectedPlaceId, // Send Google Place ID if available
            capacity: parseInt(formData.capacity),
            ticketPrice: parseFloat(formData.ticketPrice),
        };
            ...
            <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto p-0">
                ...
                <div className="px-6 space-y-4">
                    ...
                    <div className="space-y-2 relative" ref={locationRef}>
                        ...
                        <div className="relative">
                            <Input id="address" ref={inputRef} placeholder="Enter venue or address" value={locationQuery} onChange={handleLocationInputChange} onFocus={() => setShowLocationSuggestions(true)} disabled={isLoading} />
                            {isLoadingLocations && <Loader2 className="absolute right-3 top-3 h-4 w-4 animate-spin text-gray-400" />}
                        </div>

                        {showLocationSuggestions && locationQuery && locationSuggestions.length > 0 && (
                    ...
                    {/* Event Images (Multiple Upload) */}
                    <div className="space-y-2">
                        <Label className="text-sm">
                            Event Images <span className="text-destructive">*</span>
                        </Label>
                                    ...
                                    <Card key={image.id} className="relative overflow-hidden p-0">
                                        <CardContent className="p-0">
                                            <div className="relative aspect-video">
                                                ...
                                                <Button type="button" variant="destructive" size="icon" className="absolute top-2 right-2 h-6 w-6 rounded-full" onClick={() => removeImage(image.id)} disabled={isLoading}>
                                                    <X className="h-3 w-3" />
                                                </Button>
                                            </div>
                                        </CardContent>
                        ...
                        <label className={`flex flex-col items-center justify-center w-full h-24 border-2 border-dashed rounded-md cursor-pointer hover:bg-accent/50 transition-colors ${isLoading ? "opacity-50 cursor-not-allowed" : ""}`}>
                            <div className="flex flex-col items-center justify-center">
                                <Upload className="w-5 h-5 text-muted-foreground mb-1" />
                                <p className="text-xs text-muted-foreground">{imageFiles.length === 0 ? "Upload images (Max 5MB each)" : `Upload more images (${imageFiles.length} uploaded)`}</p>
                            </div>
